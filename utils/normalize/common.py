...
def get_normalization_function(method: str, feature_range: Tuple[float, float] = (0, 1)):
    """
    ...
    if method == 'minmax':
        ...
        return lambda x: apply_minmax(x, feature_range)
    elif method == 'zscore':
        ...
        return lambda x: apply_log_minmax(x, feature_range)
    else:
...
def _normalize_signal_channels(
    ...
    npe_func = get_normalization_function(channel_methods[0], feature_ranges[0])
    firstTime_func = get_normalization_function(channel_methods[1], feature_ranges[1])
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
                        ...
                        if normalize_signal_channels and channel_methods is not None:
                            ...
                            if (is_tensor(target_arg) and target_arg.dim() >= 2) or (is_array(target_arg) and target_arg.ndim >= 2):
                                if (is_tensor(target_arg) and target_arg.shape[-2] == 2) or (is_array(target_arg) and target_arg.shape[-2] == 2):
                                    ...
                                    # Always compute and store stats so they can be accessed inside the function
                                    ...
                                    normalized_arg = _normalize_signal_channels(target_arg, channel_methods, feature_ranges)
                                    
                                    # Store stats as function attribute for access inside function
                                    # Store on the original function object, not wrapper
                                    func._normalization_stats = channel_stats
            ...
            if denormalize and is_numeric_data(result):
                if normalize_signal_channels and channel_methods is not None and channel_stats is not None:
                    ...
                    if (is_tensor(result) and result.dim() >= 2) or (is_array(result) and result.ndim >= 2):
                        if (is_tensor(result) and result.shape[-2] == 2) or (is_array(result) and result.shape[-2] == 2):
                            result = _denormalize_signal_channels(result, channel_methods, channel_stats, feature_ranges)
                        else:
                            # Not a signal, use single method
