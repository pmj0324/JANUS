#!/usr/bin/env python3
"""
Training script for GENESIS using Optimal Transport Flow Matching.
Supports CFG, validation, and early stopping.
Note: This uses a simplified OT approximation (same as Rectified Flow in practice).
"""
...
def apply_config(config: dict):
    ...
    global sampling_method, sampling_steps, ot_sinkhorn_reg, ot_sinkhorn_iter
    ...
    if 'optimal_transport' in config:
        ot = config['optimal_transport']
        ot_sinkhorn_reg = ot.get('sinkhorn_reg', ot_sinkhorn_reg)
        ot_sinkhorn_iter = ot.get('sinkhorn_iter', ot_sinkhorn_iter)
...
def main():
    parser = argparse.ArgumentParser(description="Train GENESIS with Optimal Transport Flow Matching")
    ...
    # Initialize Optimal Transport Flow
    flow_matching = OptimalTransportFlow(sinkhorn_reg=ot_sinkhorn_reg, sinkhorn_iter=ot_sinkhorn_iter)
    ...
    print("="*60)
    print(f"Method: Optimal Transport Flow Matching (simplified)")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx, (sig, geo, label) in pbar:
            ...
            # Compute path x_t (OT uses simplified approximation)
            x_t = flow_matching.compute_path(x0, x1, t)
            
            # Compute ground truth velocity (OT uses simplified approximation)
        ...
        if epoch % val_every == 0:
            ...
            if improvement > early_stopping_min_delta:
                ...
                checkpoint = {
                    ...
                    "ot_sinkhorn_reg": ot_sinkhorn_reg,
                    "ot_sinkhorn_iter": ot_sinkhorn_iter,
                }
    ...
    with torch.no_grad():
        ...
        if plot_save_dir is not None:
            ...
            fig_sampled, _ = show_event_dual_plot(
                ...
                show=False,
                title_prefix=f"Optimal Transport Flow | Sampled data | using label from event {ref_idx}",
