...
# Runtime / outputs
output_dir = Path("./tasks/rectified_flow_0413_loglog_ft_linformer")
save_plots = True
seed = 42
compile_model = False
print_every = 50
...
val_ratio = 0.1
val_every = 1
resume_checkpoint = "./tasks/rectified_flow_0413_loglog_linformer/models/last.pt"
...
def save_epoch_histogram_plot(
    ...
    *,
    ...
    """Save overlaid histograms (true vs generated) in normalized/denormalized domains."""
    ...
    panels = [
        (axes[0, 0], real_npe_norm, sampled_npe_norm, "nPE (normalized)", npe_norm_min, npe_norm_max),
        (axes[0, 1], real_ftime_norm, sampled_ftime_norm, "FirstTime (normalized)", ftime_norm_min, ftime_norm_max),
        (axes[1, 0], real_npe_denorm, sampled_npe_denorm, "nPE (denorm)", npe_denorm_min, npe_denorm_max),
        (axes[1, 1], real_ftime_denorm, sampled_ftime_denorm, "FirstTime (denorm)", ftime_denorm_min, ftime_denorm_max),
    ]

    for ax, arr_true, arr_gen, title, x_min, x_max in panels:
        ax.hist(
            arr_true,
            bins=bins,
            range=(x_min, x_max),
        ...
        ax.hist(
            ...
            color="tab:orange",
            alpha=0.45,
            label=f"Generated (N={arr_gen.size})",
        )
        if log_y:
            ax.set_yscale("log")
        ax.set_title(title, fontsize=12)
...
def main():
    ...
    if resume_checkpoint is not None:
        ...
        if resume_load_optimizer and "optimizer_state_dict" in checkpoint:
            optim.load_state_dict(checkpoint["optimizer_state_dict"])
            print("Optimizer state restored.")
        else:
            print("Optimizer state not restored; using fresh optimizer.")

        scheduler_state = checkpoint.get("lr_scheduler_state_dict")
        if resume_load_optimizer and scheduler_state is not None:
            lr_scheduler.load_state_dict(scheduler_state)
            print("LR scheduler state restored.")

        scaler_state = checkpoint.get("scaler_state_dict")
        if scaler is not None and resume_load_optimizer and scaler_state is not None:
            scaler.load_state_dict(scaler_state)
            print("GradScaler state restored.")

        start_epoch = int(checkpoint.get("epoch", 0)) + 1
        best_val_loss = float(checkpoint.get("best_val_loss", best_val_loss))
        best_val_epoch = int(checkpoint.get("best_val_epoch", best_val_epoch))
        print(
            f"Resume state: last_epoch={start_epoch - 1}, "
            f"best_val_loss={best_val_loss:.6f}, best_val_epoch={best_val_epoch}"
        )
    ...
    print(f"Additional epochs this run: {num_epochs}")
    print(f"Epoch range this run: {start_epoch} -> {final_epoch}")
    ...
    print("="*60)
    print(f"\nStarting training from epoch {start_epoch} to {final_epoch}...")
    print("="*60 + "\n")
    ...
    for epoch in range(start_epoch, final_epoch + 1):
        ...
        pbar = tqdm(enumerate(train_loader, 1), total=steps_per_epoch, desc=f"Epoch {epoch}/{final_epoch} [Train]", file=sys.stdout)
        ...
        for batch_idx, (sig, geo, label) in pbar:
            ...
            current_step = (epoch - start_epoch) * steps_per_epoch + batch_idx
        ...
        if epoch % val_every == 0:
            ...
            with torch.inference_mode():
                val_pbar = tqdm(val_loader, desc=f"Epoch {epoch}/{final_epoch} [Val]", file=sys.stdout)
            ...
            print(f"\nEpoch {epoch:3d}/{final_epoch} | Train Loss: {epoch_train_loss:.6f} | Val Loss: {epoch_val_loss:.6f} | Best Val: {best_val_loss:.6f}")
            ...
            print(f"\nEpoch {epoch:3d}/{final_epoch} | Train Loss: {epoch_train_loss:.6f} | LR: {optim.param_groups[0]['lr']:.2e}")
            print("-"*60)
