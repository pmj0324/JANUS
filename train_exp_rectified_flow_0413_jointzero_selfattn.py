...
from utils.normalize import apply_minmax_geo
...
# Runtime / outputs
output_dir = Path("./tasks/rectified_flow_0413_jointzero_selfattn")
save_plots = True
seed = 42
compile_model = False
print_every = 50
...
# Pre-train signal histogram snapshot
run_signal_histogram_snapshot = True
signal_histogram_num_bins = 200
signal_histogram_chunk_size = 8192
signal_histogram_npe_shift = 0.0
...
signal_log_denoms = np.array([1.0, 1.0], dtype=np.float32)
...
def _print_signal_normalize_config():
    ...
    print(f"  nPE      : clamp [0, {npe_clip:g}] -> log1p -> divide by dataset p{signal_percentile:g}")
    print(
        f"             formula: nPE_norm = log1p(clip(nPE, 0, {npe_clip:g})) / p{signal_percentile:g}(log1p(nPE))"
    )
    print(f"  FirstTime: clamp [0, {ftime_clip:g}] -> log1p -> divide by dataset p{signal_percentile:g}")
    print(
        f"             formula: FirstTime_norm = log1p(clip(FirstTime, 0, {ftime_clip:g})) / "
        f"p{signal_percentile:g}(log1p(FirstTime))"
    )
    print(
        f"  signal log denoms: nPE={signal_log_denoms[0]:.6g}, FirstTime={signal_log_denoms[1]:.6g}"
    )
...
def _compute_signal_log_denoms(
    dataset_path: str,
    *,
    percentile: float,
    chunk_size: int = 4096,
) -> np.ndarray:
    """Compute dataset-wide log1p percentile scales for [nPE, FirstTime]."""
    ...
    with h5py.File(dataset_path, "r") as f:
        ...
        for start in range(0, total, max(1, int(chunk_size))):
            ...
            npe = np.clip(sig[:, 0, :], 0.0, npe_clip)
            ftime = np.clip(sig[:, 1, :], 0.0, ftime_clip)

            npe_active = npe[np.isfinite(npe) & (npe > 0.0)]
            ftime_active = ftime[np.isfinite(ftime) & (ftime > 0.0)]

            if npe_active.size > 0:
                npe_logs.append(np.log1p(npe_active))
            if ftime_active.size > 0:
                ftime_logs.append(np.log1p(ftime_active))

    npe_denom = float(np.percentile(np.concatenate(npe_logs), percentile)) if npe_logs else 1.0
    ftime_denom = float(np.percentile(np.concatenate(ftime_logs), percentile)) if ftime_logs else 1.0
    npe_denom = max(npe_denom, 1e-8)
    ftime_denom = max(ftime_denom, 1e-8)
    return np.array([npe_denom, ftime_denom], dtype=np.float32)
...
def _run_signal_histogram_snapshot(output_base_dir: Path):
    """Render one signal-normalization histogram figure before training starts."""
    script_dir = Path(__file__).resolve().parent
    plot_script = script_dir / "example" / "plot_signal_normalization_histograms.py"
    output_path = output_base_dir / "signal_normalization_histograms_pretrain.png"

    if not plot_script.exists():
        raise FileNotFoundError(f"signal histogram script not found: {plot_script}")
    ...
    cmd = [
        ...
        str(output_path),
        "--chunk-size",
        str(signal_histogram_chunk_size),
        "--num-bins",
        str(signal_histogram_num_bins),
        "--npe-shift",
        str(signal_histogram_npe_shift),
    ]

    print("Running pre-train signal histogram snapshot...")
    print("Command:", " ".join(cmd))
    subprocess.run(cmd, check=True, cwd=script_dir)
    print(f"Pre-train signal histogram saved to: {output_path}")
...
def prepare_batch(
    ...
    sig_norm = sig.clone()
    sig_norm[:, 0, :] = torch.log1p(sig_norm[:, 0, :]) / float(signal_log_denoms[0])
    sig_norm[:, 1, :] = torch.log1p(sig_norm[:, 1, :]) / float(signal_log_denoms[1])

    label_norm = label.clone()
    label_norm[:, 0] = (
        2.0 * (torch.log1p(label_norm[:, 0]) - energy_log_min) / (energy_log_max - energy_log_min + 1e-8) - 1.0
    )
    for idx, stats in zip((3, 4, 5), _LABEL_XYZ_MINMAX):
        label_norm[:, idx] = (
            2.0 * (label_norm[:, idx] - stats["min"]) / (stats["max"] - stats["min"] + 1e-8) - 1.0
        )
...
def denormalize_sig(sig: torch.Tensor) -> torch.Tensor:
    ...
    out = sig.clone()
    if sig.dim() == 3:
        out[:, 0, :] = torch.expm1(sig[:, 0, :] * float(signal_log_denoms[0]))
        out[:, 1, :] = torch.expm1(sig[:, 1, :] * float(signal_log_denoms[1]))
    else:
        out[0, :] = torch.expm1(sig[0, :] * float(signal_log_denoms[0]))
        out[1, :] = torch.expm1(sig[1, :] * float(signal_log_denoms[1]))
    return out
...
def main():
    ...
    if run_signal_histogram_snapshot:
        histogram_output_dir = plot_save_dir if plot_save_dir is not None else output_dir
        _run_signal_histogram_snapshot(histogram_output_dir)
    else:
        print("Pre-train signal histogram snapshot: disabled")
    ...
    signal_log_denoms = _compute_signal_log_denoms(
        h5_path,
        percentile=signal_percentile,
        chunk_size=signal_histogram_chunk_size,
    )
    print(
        f"Computed signal log denoms from dataset p{signal_percentile:g}: "
        f"nPE={signal_log_denoms[0]:.6g}, FirstTime={signal_log_denoms[1]:.6g}"
    )
        ...
        if epoch % val_every == 0:
            ...
            if improvement > early_stopping_min_delta:
                ...
                checkpoint = {
                    ...
                    "signal_log_denoms": signal_log_denoms.copy(),
